...
ShowCasinoGamesMenu(playerid, dialogid)
{
	switch(dialogid)
	{
		...
		{
			return ShowPlayerDialog(playerid, DIALOG_CGAMESCREDITS, DIALOG_STYLE_MSGBOX, "{FFFFFF}Casino Games - (Creadits)", "{FFFFFF}Chinh sua boi: Hades", "Quay Lai", "");
		}
	...
	if(GetPVarInt(playerid, "HelpTime") > 0)
 	{
    	...
    	if(GetPVarInt(playerid, "HelpTime") == 0)
     	{
      		SendClientMessageEx(playerid, COLOR_GREY, "Yeu cau tro giup cua ban da het han. Neu day la mot loi hay tham gia dien dan Hades de bao loi.");
